...
async function getRandomPhoto() {
    const apiKey = 'qrAPbcLieJ2JU9Ne3OCqawHaVEnQg63ZELYotPpYsIo';
    try {
...
async function renderPhoto() {
    ...
    if (photo) {
        ...
        const imagePhotographerNameDiv = document.querySelector('.photographer_info_name');
        imagePhotographerNameDiv.textContent = `${photo.user.name}`;

        const imageLikesCounterSpan = document.querySelector('.image_likes_counter');
        imageLikesCounterSpan.textContent = `${photo.likes}`;

    }
}

const counterButton = document.querySelector('.image_likes_button');
counterButton.addEventListener('click', function () {
    increaseCounter();
});

function increaseCounter() {
    const likesCounter = document.querySelector('.image_likes_counter');
